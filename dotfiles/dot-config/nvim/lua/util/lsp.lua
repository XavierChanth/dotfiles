...
function M.rename_file()
  ...
  local root = assert(require("util.root").realpath(require("util.root").root({ normalize = true })))
  ...
  vim.ui.input({
    ...
    new = require("util.lazy").norm(root .. "/" .. new)
  ...
  M.on_supports_method("textDocument/documentHighlight", function(_, buf)
    vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
      ...
      callback = function(ev)
        if not require("util.lsp_keymaps").has(buf, "documentHighlight") then
          return false
        end
