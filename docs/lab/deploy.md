...
Routine commands need a Nix-capable Linux or macOS client, SSH credentials for `chant`, and chant's real sudo password on the target. Root SSH remains disabled. `interactiveSudo` gives the password directly to remote sudo; this repository neither stores it nor grants NOPASSWD. Target-side `remoteBuild` is enabled, so the target must trust/signature-accept everything needed to realize the closure. No `trusted-users` change is made.
